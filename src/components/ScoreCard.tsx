...
export default function ScoreCard({ username, score, rank, onClose, color = '#00ffcc' }: ScoreCardProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `theglobalword-score-${username.toLowerCase().replace(/\s+/g, '-')}.png`, { type: 'image/png' });

      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: `The Global Word - Score`,
          text: rank ? `Je suis Top ${rank} mondial au défi The Global Word avec ${score} pts ! 🌍🏆` : `J'ai marqué ${score} pts au défi The Global Word ! 🌍🏆`,
          url: 'https://theglobalword.org',
          files: [file],
        });
      } else {
        const link = document.createElement('a');
        link.download = `theglobalword-score-${username.toLowerCase().replace(/\s+/g, '-')}.png`;
        link.href = dataUrl;
        link.click();
      }
    } catch (err) {
      ...
      <div className="relative max-w-sm w-full">
        ...
        >
          ...
          <div className="flex justify-between items-start z-10">
            <div>
              <p className="text-white/50 text-[10px] font-bold tracking-[0.2em] uppercase mb-1">Score Mondial</p>
              <h2 className="text-2xl font-black text-white tracking-tight leading-none truncate max-w-[200px]">{username}</h2>
            </div>
            <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center bg-white/5 backdrop-blur-sm text-yellow-400">
              <Trophy size={18} />
            </div>
          </div>

          {/* Center Content */}
          <div className="flex-1 flex flex-col items-center justify-center z-10 py-4 w-full gap-4">
            {rank && (
              <div className="flex flex-col items-center">
                <span className="text-white/40 uppercase tracking-[0.3em] text-xs mb-1 font-bold">Classement</span>
                <span className="text-4xl font-black text-white" style={{ textShadow: `0 0 20px rgba(255,255,255,0.3)` }}>
                  TOP {rank}
                </span>
              </div>
            )}
            <div className="flex flex-col items-center">
              <span className="text-white/40 uppercase tracking-[0.3em] text-xs mb-1 font-bold">Points</span>
              <span 
                className="text-6xl font-black text-transparent bg-clip-text"
                style={{
                  backgroundImage: `linear-gradient(to bottom right, #fff, ${color})`,
